...
RUN 	yum -y install gcc-c++ wget GeoIP-devel GeoIP GeoIP-data unzip make python python-pip curl freetype openssl-devel pcre-devel zlib-devel ca-certificates python-devel perl &&\
	cd /usr/src && wget https://www.openssl.org/source/openssl-${OPENSSL_VERSION}.tar.gz && tar -zxf openssl-${OPENSSL_VERSION}.tar.gz && \
	cd /usr/src &&\
	...
	rm -rf /usr/src/ngx_pagespeed-* && \
	rm -rf /usr/src/nginx-* && \
	rm -rf /usr/src/*.tar.gz && rm -rf /usr/src/*.zip &&\
